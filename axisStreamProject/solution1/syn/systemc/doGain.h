// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2014.2
// Copyright (C) 2014 Xilinx Inc. All rights reserved.
// 
// ===========================================================

#ifndef _doGain_HH_
#define _doGain_HH_

#include "systemc.h"
#include "AESL_pkg.h"

#include "doGain_mul_32s_32s_32_6.h"
#include "doGain_CRTL_BUS_s_axi.h"

namespace ap_rtl {

template<unsigned int C_S_AXI_CRTL_BUS_ADDR_WIDTH = 5,
         unsigned int C_S_AXI_CRTL_BUS_DATA_WIDTH = 32>
struct doGain : public sc_module {
    // Port declarations 38
    sc_in< sc_logic > s_axi_CRTL_BUS_AWVALID;
    sc_out< sc_logic > s_axi_CRTL_BUS_AWREADY;
    sc_in< sc_uint<C_S_AXI_CRTL_BUS_ADDR_WIDTH> > s_axi_CRTL_BUS_AWADDR;
    sc_in< sc_logic > s_axi_CRTL_BUS_WVALID;
    sc_out< sc_logic > s_axi_CRTL_BUS_WREADY;
    sc_in< sc_uint<C_S_AXI_CRTL_BUS_DATA_WIDTH> > s_axi_CRTL_BUS_WDATA;
    sc_in< sc_uint<C_S_AXI_CRTL_BUS_DATA_WIDTH/8> > s_axi_CRTL_BUS_WSTRB;
    sc_in< sc_logic > s_axi_CRTL_BUS_ARVALID;
    sc_out< sc_logic > s_axi_CRTL_BUS_ARREADY;
    sc_in< sc_uint<C_S_AXI_CRTL_BUS_ADDR_WIDTH> > s_axi_CRTL_BUS_ARADDR;
    sc_out< sc_logic > s_axi_CRTL_BUS_RVALID;
    sc_in< sc_logic > s_axi_CRTL_BUS_RREADY;
    sc_out< sc_uint<C_S_AXI_CRTL_BUS_DATA_WIDTH> > s_axi_CRTL_BUS_RDATA;
    sc_out< sc_lv<2> > s_axi_CRTL_BUS_RRESP;
    sc_out< sc_logic > s_axi_CRTL_BUS_BVALID;
    sc_in< sc_logic > s_axi_CRTL_BUS_BREADY;
    sc_out< sc_lv<2> > s_axi_CRTL_BUS_BRESP;
    sc_in_clk ap_clk;
    sc_in< sc_logic > ap_rst_n;
    sc_in< sc_lv<32> > inStream_TDATA;
    sc_in< sc_logic > inStream_TVALID;
    sc_out< sc_logic > inStream_TREADY;
    sc_in< sc_lv<4> > inStream_TKEEP;
    sc_in< sc_lv<4> > inStream_TSTRB;
    sc_in< sc_lv<2> > inStream_TUSER;
    sc_in< sc_lv<1> > inStream_TLAST;
    sc_in< sc_lv<5> > inStream_TID;
    sc_in< sc_lv<6> > inStream_TDEST;
    sc_out< sc_lv<32> > outStream_TDATA;
    sc_out< sc_logic > outStream_TVALID;
    sc_in< sc_logic > outStream_TREADY;
    sc_out< sc_lv<4> > outStream_TKEEP;
    sc_out< sc_lv<4> > outStream_TSTRB;
    sc_out< sc_lv<2> > outStream_TUSER;
    sc_out< sc_lv<1> > outStream_TLAST;
    sc_out< sc_lv<5> > outStream_TID;
    sc_out< sc_lv<6> > outStream_TDEST;
    sc_out< sc_logic > interrupt;


    // Module declarations
    doGain(sc_module_name name);
    SC_HAS_PROCESS(doGain);

    ~doGain();

    sc_trace_file* mVcdFile;

    ofstream mHdltvinHandle;
    ofstream mHdltvoutHandle;
    doGain_CRTL_BUS_s_axi<C_S_AXI_CRTL_BUS_ADDR_WIDTH,C_S_AXI_CRTL_BUS_DATA_WIDTH>* doGain_CRTL_BUS_s_axi_U;
    doGain_mul_32s_32s_32_6<0,6,32,32,32>* doGain_mul_32s_32s_32_6_U0;
    sc_signal< sc_logic > ap_start;
    sc_signal< sc_logic > ap_done;
    sc_signal< sc_logic > ap_idle;
    sc_signal< sc_lv<2> > ap_CS_fsm;
    sc_signal< sc_logic > ap_ready;
    sc_signal< sc_lv<32> > gain;
    sc_signal< sc_logic > doGain_CRTL_BUS_s_axi_U_ap_dummy_ce;
    sc_signal< sc_lv<10> > idx_reg_123;
    sc_signal< sc_lv<1> > exitcond_fu_135_p2;
    sc_signal< sc_lv<1> > exitcond_reg_186;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it0;
    sc_signal< bool > ap_sig_bdd_91;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it1;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it2;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it3;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it4;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond_reg_186_pp0_it4;
    sc_signal< sc_logic > ap_sig_ioackin_outStream_TREADY;
    sc_signal< sc_logic > ap_reg_ppiten_pp0_it5;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond_reg_186_pp0_it1;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond_reg_186_pp0_it2;
    sc_signal< sc_lv<1> > ap_reg_ppstg_exitcond_reg_186_pp0_it3;
    sc_signal< sc_lv<10> > idx_1_fu_141_p2;
    sc_signal< sc_lv<4> > tmp_keep_V_reg_200;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_keep_V_reg_200_pp0_it1;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_keep_V_reg_200_pp0_it2;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_keep_V_reg_200_pp0_it3;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_keep_V_reg_200_pp0_it4;
    sc_signal< sc_lv<4> > tmp_strb_V_reg_205;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_strb_V_reg_205_pp0_it1;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_strb_V_reg_205_pp0_it2;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_strb_V_reg_205_pp0_it3;
    sc_signal< sc_lv<4> > ap_reg_ppstg_tmp_strb_V_reg_205_pp0_it4;
    sc_signal< sc_lv<2> > tmp_user_V_reg_210;
    sc_signal< sc_lv<2> > ap_reg_ppstg_tmp_user_V_reg_210_pp0_it1;
    sc_signal< sc_lv<2> > ap_reg_ppstg_tmp_user_V_reg_210_pp0_it2;
    sc_signal< sc_lv<2> > ap_reg_ppstg_tmp_user_V_reg_210_pp0_it3;
    sc_signal< sc_lv<2> > ap_reg_ppstg_tmp_user_V_reg_210_pp0_it4;
    sc_signal< sc_lv<1> > tmp_last_V_reg_215;
    sc_signal< sc_lv<1> > ap_reg_ppstg_tmp_last_V_reg_215_pp0_it1;
    sc_signal< sc_lv<1> > ap_reg_ppstg_tmp_last_V_reg_215_pp0_it2;
    sc_signal< sc_lv<1> > ap_reg_ppstg_tmp_last_V_reg_215_pp0_it3;
    sc_signal< sc_lv<1> > ap_reg_ppstg_tmp_last_V_reg_215_pp0_it4;
    sc_signal< sc_lv<5> > tmp_id_V_reg_220;
    sc_signal< sc_lv<5> > ap_reg_ppstg_tmp_id_V_reg_220_pp0_it1;
    sc_signal< sc_lv<5> > ap_reg_ppstg_tmp_id_V_reg_220_pp0_it2;
    sc_signal< sc_lv<5> > ap_reg_ppstg_tmp_id_V_reg_220_pp0_it3;
    sc_signal< sc_lv<5> > ap_reg_ppstg_tmp_id_V_reg_220_pp0_it4;
    sc_signal< sc_lv<6> > tmp_dest_V_reg_225;
    sc_signal< sc_lv<6> > ap_reg_ppstg_tmp_dest_V_reg_225_pp0_it1;
    sc_signal< sc_lv<6> > ap_reg_ppstg_tmp_dest_V_reg_225_pp0_it2;
    sc_signal< sc_lv<6> > ap_reg_ppstg_tmp_dest_V_reg_225_pp0_it3;
    sc_signal< sc_lv<6> > ap_reg_ppstg_tmp_dest_V_reg_225_pp0_it4;
    sc_signal< sc_lv<32> > grp_fu_175_p2;
    sc_signal< sc_logic > ap_reg_ioackin_outStream_TREADY;
    sc_signal< sc_lv<32> > grp_fu_175_p0;
    sc_signal< sc_lv<32> > grp_fu_175_p1;
    sc_signal< sc_logic > doGain_ap_rst;
    sc_signal< sc_logic > grp_fu_175_ce;
    sc_signal< sc_lv<2> > ap_NS_fsm;
    static const sc_logic ap_const_logic_1;
    static const sc_logic ap_const_logic_0;
    static const sc_lv<2> ap_ST_st1_fsm_0;
    static const sc_lv<2> ap_ST_pp0_stg0_fsm_1;
    static const sc_lv<2> ap_ST_st8_fsm_2;
    static const int C_DATA_WIDTH;
    static const sc_lv<1> ap_const_lv1_0;
    static const sc_lv<10> ap_const_lv10_0;
    static const sc_lv<10> ap_const_lv10_3E8;
    static const sc_lv<10> ap_const_lv10_1;
    // Thread declarations
    void thread_ap_clk_no_reset_();
    void thread_ap_done();
    void thread_ap_idle();
    void thread_ap_ready();
    void thread_ap_sig_bdd_91();
    void thread_ap_sig_ioackin_outStream_TREADY();
    void thread_doGain_CRTL_BUS_s_axi_U_ap_dummy_ce();
    void thread_doGain_ap_rst();
    void thread_exitcond_fu_135_p2();
    void thread_grp_fu_175_ce();
    void thread_grp_fu_175_p0();
    void thread_grp_fu_175_p1();
    void thread_idx_1_fu_141_p2();
    void thread_inStream_TREADY();
    void thread_outStream_TDATA();
    void thread_outStream_TDEST();
    void thread_outStream_TID();
    void thread_outStream_TKEEP();
    void thread_outStream_TLAST();
    void thread_outStream_TSTRB();
    void thread_outStream_TUSER();
    void thread_outStream_TVALID();
    void thread_ap_NS_fsm();
    void thread_hdltv_gen();
};

}

using namespace ap_rtl;

#endif
