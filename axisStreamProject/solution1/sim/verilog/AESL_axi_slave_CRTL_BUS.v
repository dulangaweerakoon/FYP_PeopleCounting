...
reg  BREADY_reg;
reg [gain_c_bitwidth - 1 : 0] mem_gain [gain_DEPTH - 1 : 0];
reg gain_write_data_finish;
reg gain_write_data_finish_tmp;
...
always @(gain_write_data_finish_tmp or TRAN_CRTL_BUS_start_in) begin
    if (gain_write_data_finish_tmp == 1) begin
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_gain_data_done === 1) begin
            ...
            if (write_gain_count == gain_DEPTH) begin
                write_gain_run_flag <= 0; 
            end
        end
    end
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_gain_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    write (gain_data_in_addr + i * 4, gain_data_tmp_reg, write_gain_resp);
                end
                gain_write_data_finish_tmp <= 1;
                process_busy = 0;
...
initial begin : read_gain_file_process 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < gain_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", mem_gain[i]); 
          if (ret != 1) begin 
